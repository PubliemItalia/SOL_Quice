...
$tendSoc = "SELECT DISTINCT azienda_prodotto FROM qui_righe_rda WHERE stato_ordine = '3'";
}
...
//tendina negozi
if ($clausole > 0) {
$tendSHOP = "SELECT DISTINCT negozio FROM qui_righe_rda WHERE stato_ordine = '3' AND ".$queryaccessoria;
} else {
$tendSHOP = "SELECT DISTINCT negozio FROM qui_righe_rda WHERE stato_ordine = '3'";
}
...
//tendina unità
if ($clausole > 0) {
$tendUNI = "SELECT DISTINCT id_unita, nome_unita FROM qui_righe_rda WHERE stato_ordine = '3' AND ".$queryaccessoria;
} else {
$tendUNI = "SELECT DISTINCT id_unita, nome_unita FROM qui_righe_rda WHERE stato_ordine = '3'";
}
...
if ($clausole > 0) {
$tendCATEG = "SELECT DISTINCT categoria FROM qui_righe_rda WHERE stato_ordine = '3' AND ".$queryaccessoria;
} else {
$tendCATEG = "SELECT DISTINCT categoria FROM qui_righe_rda WHERE stato_ordine = '3'";
}
	  ...
	  switch ($file_presente) {
		case "report_scorte_magazzino.php":
		  $tab_output .= '<div  style="width: 100%; float: left; height: 20px;">
			...
			<div id="scelta_categoria" style="width: 100%; float: left; height: 39px;">
			Selezionare un negozio';
			/*<select name="categoria_ricerca" class="codice_lista_nopadding" id="categoria_ricerca" style="height:27px; width:90%;">
			<option selected value="">Seleziona negozio</option>';
		  if ($shopDaModulo == "") {
			...
			if ($rigat[categoria1_it] == $categoria_ricercaDaModulo) {
			  $tab_output .= "<option selected value=".$rigat[categoria1_it].">".ucfirst(str_replace("_"," ",$rigat[categoria1_it]))."</option>";
			} else {
			  $tab_output .= "<option value=".$rigat[categoria1_it].">".ucfirst(str_replace("_"," ",$rigat[categoria1_it]))."</option>";
			}
			}
			$tab_output .= '</select>';*/
		  $tab_output .= '</div';
		...
		case "report_righe_admin.php":
		  ...
		  <div id="divcat" style="width: 100%; float: left; height: 39px;">'.$tendina_categoria.'</select>';
		  $tab_output .= '</div>';
		break;
		case "ordini_sap.php":
		case "report_rda.php":
