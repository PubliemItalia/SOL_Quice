...
if ($negozio_riga == "labels") {
  ...
  while ($rigar = mysql_fetch_array($risultr)) {
	...
	if ($rigar[ric_mag] == "maglab") {
	  $sqlk = "SELECT * FROM qui_pharma_quant_prezzi WHERE tipologia = '$rigar[extra]'";
	  $risultk = mysql_query($sqlk) or die("Impossibile eseguire l'interrogazione" . mysql_error());
	  while ($rigak = mysql_fetch_array($risultk)) {
		  $prezzo_prodotto = $rigak[prezzo];
		}
	}
  }
}
...
mysql_query("INSERT INTO qui_righe_rda (azienda_prodotto, azienda_utente, flusso, nome_company, cappello, lingua_impostata, cavo, dest_contab, id_carrello, negozio, id_unita, nome_unita, categoria, id_utente, id_resp, id_prodotto, codice_art, descrizione, confezione, quant, quant_modifica, prezzo, totale, data_inserimento, data_ultima_modifica, id_rda, pack_list, stato_ordine, flag_buyer, flag_chiusura, flag_packing_list, output_mode, file_sap, n_ord_sap, n_fatt_sap, evaso_magazzino, vecchio_codice, report_select, gruppo_merci, wbs, ordine_stampa, id_buyer, id_magazz, nazione) SELECT azienda_prodotto, azienda_utente, flusso, nome_company, cappello, lingua_impostata, cavo, dest_contab, id_carrello, negozio, id_unita, nome_unita, categoria, id_utente, id_resp, id_prodotto, codice_art, descrizione, confezione, quant, quant_modifica, prezzo, totale, data_inserimento, data_ultima_modifica, id_rda, pack_list, stato_ordine, flag_buyer, flag_chiusura, flag_packing_list, output_mode, file_sap, n_ord_sap, n_fatt_sap, evaso_magazzino, vecchio_codice, report_select, gruppo_merci, wbs, ordine_stampa, id_buyer, id_magazz, nazione FROM qui_righe_rda WHERE id = '".$id_riga."'") or die("Impossibile eseguire l'inserimento 1" . mysql_error());
