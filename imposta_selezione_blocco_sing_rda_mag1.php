...
$array_rda_exist = array();
$queryv = "SELECT * FROM qui_righe_rda WHERE flag_buyer = '3' AND stato_ordine < '4' AND output_mode = 'mag'";
$resultv = mysql_query($queryv) or die("Impossibile eseguire l'interrogazione4" . mysql_error());
while ($rowv = mysql_fetch_array($resultv)) {
if (!in_array($rowv[id_rda],$array_rda_exist)) {
$add_rda = array_push($array_rda_exist,$rowv[id_rda]);
...
$sqlk = "SELECT * FROM qui_righe_rda WHERE id_rda = '$id_rda' AND output_mode = 'mag' AND evaso_magazzino = '0'";
  ...
  if ($rigan[evaso_magazzino] == 0) {
    /*if ($flag_ricerca != "") {
      switch ($rigan[flag_buyer]) {
        case "2":
          echo "<input name=id_riga[] type=checkbox id=id_riga[] value=".$rigan[id]." onClick=\"raccolta(".$rigan[id].",'3',".$id_rda.");\">";
        break;
        case "3":
          echo "<input name=id_riga[] type=checkbox id=id_riga[] checked value=".$rigan[id]." onClick=\"raccolta(".$rigan[id].",'2',".$id_rda.");\">";
        break;
      }
    } else {*/
      switch ($rigan[flag_buyer]) {
