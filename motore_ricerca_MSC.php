...
if (in_array($file_presente,$array_societa)) {
	  switch ($file_presente) {
		  ...
		  case "report_fatturazione.php":
			if ($clausole > 0) {
			  $tendSoc = "SELECT DISTINCT azienda_prodotto FROM qui_righe_rda WHERE dest_contab LIKE '%".$doc."%' AND pack_list != '0' AND (output_mode = 'mag' OR output_mode = 'lab') AND ".$queryaccessoria;
			} else {
			  $tendSoc = "SELECT DISTINCT azienda_prodotto FROM qui_righe_rda WHERE dest_contab LIKE '%".$doc."%' AND pack_list != '0' AND (output_mode = 'mag' OR output_mode = 'lab')";
			}
		  break;
		  case "ordini_sap.php":
