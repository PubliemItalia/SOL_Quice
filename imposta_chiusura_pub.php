...
$lingua = $_GET['lang'];
$data_attuale = mktime();
$query = "UPDATE qui_righe_rda SET flag_buyer = '2', evaso_magazzino = '1', id_magazz = '$id_magazz', data_ultima_modifica = '$data_attuale' WHERE pack_list = '$pl' AND flag_buyer = '3' AND output_mode = 'lab'";
...
//costruzione query
if ($clausole > 0) {
$testoQuery = "SELECT * FROM qui_righe_rda WHERE stato_ordine = '3' AND output_mode = 'lab' AND evaso_magazzino = '0' AND ";
$sumquery =   "SELECT SUM(totale) as somma FROM qui_righe_rda WHERE stato_ordine = '3' AND output_mode = 'lab' AND evaso_magazzino = '0' AND ";
...
$testoQuery = "SELECT * FROM qui_righe_rda WHERE stato_ordine = '3' AND output_mode = 'lab' AND evaso_magazzino = '0'";
$sumquery =   "SELECT SUM(totale) as somma FROM qui_righe_rda WHERE stato_ordine = '3' AND output_mode = 'lab' AND evaso_magazzino = '0'";
}
...
///////////////////////////////////////////////
  ...
  */
  ...
  $tab_output .= '<div class="sel_all" id="sel_all_'.$sing_rda.'">';
  if ($flag_ricerca == "") {
	if ($stampa != "1") {
  ...
  $tab_output .= '</div>';
  $tab_output .= '<div class="stato_rda" style="width:35px; height:21px; padding-top:7px;">';
  $tab_output .= '</div>';
  $tab_output .= '</div>';
...
if ($stampa != "1") {
  ...
  if ($num_pack > 0) {
  $tab_output .= '<div id="pack_'.$sing_rda.'" style="width:320px; float:left; height:auto; border-bottom:1px solid #CCC; margin-bottom:10px;">';
	while ($rigap = mysql_fetch_array($risultp)) {
	  $tab_output .= '<div style="width:310px; min-height:15px; height:auto; overflow: hidden;">';
		...
		while ($rigab = mysql_fetch_array($risultb)) {
		  if ($rigab[n_ord_sap] == "") {
			...
			$tab_output .= '</div>';
			$tab_output .= '<a href="javascript:void(0);" onclick="elimina_pl('.$rigap[pl].','.$sing_rda.');">';
  ...
  if ($contatore_righe_flag > 0) {
	$tab_output .= "<a href=\"javascript:void(0);\" onclick=\"PopupCenter('packing_list.php?id_rda=".$sing_rda."&lang=".$lingua."&id_utente=".$id_utente."&mode=nuovo&behav=publiem', 'myPop1',800,800);\">";
  ...
  */
  ...
  $tab_output .= "<div class=columns_righe2>";
  $tab_output .= '<div id="confez5_riga" style="padding-left:10px;">';
  $tab_output .= "<strong>TOTALI</strong>";
  //fine div codice riga
  $tab_output .= '</div>';
  //div descrizione riga
  $tab_output .= '<div class="descr4_riga" style="width:400px;">';
  ...
  //div quant riga
  $tab_output .= '<div class="price6_riga_quant">';
  $tab_output .= intval($totale_quant_rda);
  $tab_output .= "</div>";
//div pulsante per evasione parziale riga
$tab_output .= '<div class="lente_prodotto">';
//fine div pulsante per evasione parziale riga
...
//div pulsante per visualizzare scheda
  ...
  //div checkbox (vuoto)
  $tab_output .= '<div class="sel_all_riga">';
  //fine div checkbox
  $tab_output .= "</div>";
  	$tab_output .= '<div class="sel_all_riga" style="width:auto; padding:12px 5px 0px 0px;">';
	$tab_output .= "</div>";
