...
///////////////////////////////////////////////
$_SESSION[percorso_indietro] = "scheda_visuale_etich.php?categoria1=".$categoria1."&categoria2=".$categoria2."&categoria3=".$categoria3."&categoria4=".$categoria4."&paese=".$paese."&negozio=".$negozio."&lang=".$lingua;
?>
...
if ($_GET['codice_art'] != "") {
  ...
  if ($pos_report > 0) {
	...
	echo "<a href=\"".$percorso_back."\">";
	  echo "<div style=\"width: auto; height: 20px; float:right; font-size:14px; font-weight:bold; text-align:right; text-decoration:none;\">";
	  echo "BACK";
	  echo "</div>";
	echo "</a>";
  echo "</div>";
...
while ($rigaq = mysql_fetch_array($risultq)) {
	...
	if ($rigaq[rif_famiglia] == "") {
				switch ($_SESSION[lang]) {
				case "it":
				$categ3 = $rigaq[categoria3_it];
				$descriz2 = $rigaq[descrizione2_it];
				$categ4 = $rigaq[categoria4_it];
				break;
				case "en":
				if ($rigaq[categoria3_en] != "") {
				$categ3 = $rigaq[categoria3_en];
				} else {
				$categ3 = $rigaq[categoria3_it];
				}
				if ($rigaq[descrizione2_en] != "") {
				$descriz2 = $rigaq[descrizione2_en];
				} else {
				$descriz2 = $rigaq[descrizione2_it];
				}
				if ($rigaq[categoria4_en] != "") {
				$categ4 = $rigaq[categoria4_en];
				} else {
				$categ4 = $rigaq[categoria4_it];
				}
				break;
		...
		echo "<div class=Titolo_famiglia>".str_replace("_"," ",$categ3)."</div>";
		echo "<div id=componente_descrizione>";
		echo "<div class=descr_famiglia>".stripslashes($descriz2)."</div>";
		echo "<div id=variante class=variante_testuale>".stripslashes($categ4)."<br>";
		echo "</div>";
		echo "</div>";
...
echo "</span>";
		...
		if ($vis_admin == 1) {
			echo "<a href=".$modulo."?action=edit&id=".$rigaq[id]."&negozio=".$negozio."&lang=".$lingua."><span class=pulsante_admin>Admin</span></a>";
		}
		echo "</div>"; 
/*		echo "<div class=comandi>";
...
//echo "id: ".$rigaq[id]."<br>";
		...
		echo "</div>"; 
		//********************************************************
		//fine if elementi famiglia senza dipendenze, (tutti elementi diversi tra loro)
		//***********************************************************
	} else {
		
			
			$lista_dipendenze = $rigaq[id]."#".$rigaq[rif_famiglia];
			$pos_dip = strpos($lista_dipendenze,",");
				$array_dipendenze = explode("#",$lista_dipendenze);
				foreach ($array_dipendenze as $singola_dip) {
				$sqlm = "SELECT * FROM qui_prodotti_".$negozio." WHERE id = '$singola_dip'";
				$risultm = mysql_query($sqlm) or die("Impossibile eseguire l'interrogazione" . mysql_error());
				while ($rigam = mysql_fetch_array($risultm)) {
				switch ($_SESSION[lang]) {
				case "it":
				$categ3 = $rigam[categoria3_it];
				$descriz2 = $rigam[descrizione2_it];
				$categ4 = $rigam[categoria4_it];
				break;
				case "en":
				if ($rigam[categoria3_en] != "") {
				$categ3 = $rigam[categoria3_en];
				} else {
				$categ3 = $rigam[categoria3_it];
				}
				if ($rigam[descrizione2_en] != "") {
				$descriz2 = $rigam[descrizione2_en];
				} else {
				$descriz2 = $rigam[descrizione2_it];
				}
				if ($rigam[categoria4_en] != "") {
				$categ4 = $rigam[categoria4_en];
				} else {
				$categ4 = $rigam[categoria4_it];
				}
				break;
		...
		echo "<div id=riquadro_prodotto>";
						...
						echo "<div class=Titolo_famiglia>".str_replace("_"," ",$categ3)."</div>";
						echo "<div class=descr_famiglia>".stripslashes($descriz2)."</div>";
						echo "<div id=variante class=Titolo_famiglia>".stripslashes($categ4)."</div>";
						echo "</div>";
						//echo "<div id=risultato>";
						echo "<div id=componente_dati>";
						...
						echo "<div class=scritte_bottoncini>".$code."</div>"; 
						echo "<div class=bottoncini>".$rigam[codice_art]."</div>";
		echo "<div class=scritte_bottoncini>".$grmerci."</div>"; 
		echo "<div class=bottoncini>".$rigaq[gruppo_merci]."</div>";
						echo "<div class=scritte_bottoncini>".$price."</div>"; 
						echo "<div class=bottoncini>".number_format($rigam[prezzo],2,",",".")."</div>";
						echo "<div class=scritte_bottoncini>".$package."</div>"; 
						echo "<div class=bottoncini>".$rigam[confezione]." ".$pcs."</div>";
/*						echo "<div class=scritte_bottoncini>Gruppo merci</div>"; 
						echo "<div class=bottoncini>".$rigam[gruppo_merci]."</div>";
*/						echo "</div>";
						...
						echo "<div id=componente_immagine>";
						echo "<img src=files/".$rigam[foto]." width=248 height=248>";
						echo "</div>";
		//componente bottoni
		echo "<div id=componente_bottoni>";
		echo "<div class=comandi>";
...
/*	$titolo = str_replace("_"," ",$rigaq[categoria3_it]);
	$immagine = $rigaq[foto_famiglia];	
	$codice = $rigaq[codice_art];
	$paese = stripslashes($rigaq[paese]);
	$confezione = $rigaq[confezione];
	$prezzo = $rigaq[prezzo];
	$gruppo_merci = $rigaq[gruppo_merci];
	$descrizione_famiglia = stripslashes($rigaq[descrizione2_it]);
	$descrizione = stripslashes($rigaq[categoria4_it]);
	$immagine_fam = $rigaq[foto];
*/
...
if(mode=="") {
	...
	var tend = '<?php echo $id_back; ?>';
}
						//alert(tend);
				$.ajax({
						type: "GET",   
						url: "aggiorna_dati.php",   
						data: "id="+tend+"&negozio="+"<?php echo $negozio; ?>"+"&lang="+"<?php echo $lingua; ?>",
				...
				$.ajax({
						...
						data: "id="+tend+"&negozio="+"<?php echo $negozio; ?>"+"&categoria1="+"<?php echo $categoria1; ?>"+"&categoria2="+"<?php echo $categoria2; ?>"+"&categoria3="+"<?php echo $categoria3; ?>"+"&lang="+"<?php echo $lingua; ?>",
						success: function(output) {
