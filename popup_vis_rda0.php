...
*/	  if ($row[negozio] == "labels") {
  ...
  echo '</div>';
  echo '<div style="width:160px; height:20px; float:left;">';
	if ($row[flag_chiusura] == 1) {
	  switch ($row[output_mode]) {
		  case "mag":
		  case "lab":
			$queryf = "SELECT * FROM qui_packing_list WHERE id = '$row[pack_list]'";
			$resultf = mysql_query($queryf);
			while ($rowf = mysql_fetch_array($resultf)) {
			  if ($rowf[data_spedizione] > 0) {
				  echo date("d.m.Y",$rowf[data_spedizione]).' - ';
			  }
			}
			echo "PL ".$row[pack_list];
		  break;
		  case "sap":
			if ($row[data_chiusura] > 0) {
				echo date("d.m.Y",$row[data_chiusura]).' - ';
			}
			echo "ord. ".$row[ord_fornitore].' '.$row[fornitore_tx];
		  break;
	  }
	} else {
		echo 'In attesa di evasione';
	}
  echo '</div>';
