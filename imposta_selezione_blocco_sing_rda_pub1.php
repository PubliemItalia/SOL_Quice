...
$queryv = "SELECT * FROM qui_righe_rda WHERE flag_buyer = '3' AND stato_ordine < '4' AND output_mode = 'lab'";
...
//if ((in_array($rda_selez,$array_rda_exist)) OR ($elementi_array == 0)) {
$query = "UPDATE qui_righe_rda SET flag_buyer = '$check', id_magazz = '$id_magazz' WHERE id_rda = '$id_rda' AND output_mode = 'lab'";
if (mysql_query($query)) {
} else {
$tab_output .= "Errore durante l'inserimento: ".mysql_error();
}
...
//*******************************************
//*******************************************
  $sqln = "SELECT * FROM qui_righe_rda WHERE id_rda = '$id_rda' AND stato_ordine = '3' AND output_mode = 'lab' AND flag_chiusura = '0' AND evaso_magazzino = '0'";
  if ($clausole > 0) {
  if ($a != "") {
  $sqln .= " AND ".$a;
  }
...
if ($stampa != "1") {
  ...
  if ($contatore_righe_flag > 0) {
	$tab_output .= "<a href=\"javascript:void(0);\" onclick=\"PopupCenter('packing_list.php?id_rda=".$id_rda."&lang=".$lingua."&id_utente=".$id_utente."&mode=nuovo&behav=publiem', 'myPop1',800,800);\">";
