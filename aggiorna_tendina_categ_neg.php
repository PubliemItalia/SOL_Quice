...
$output .= "<option selected value=\"\">Tutte</option>";
    ...
    if ($shop == "") {
	  $sqlt = "SELECT DISTINCT categoria FROM qui_righe_rda WHERE stato_ordine = '4' AND ".$blocco_negozi_buyer." ORDER BY categoria ASC";
	} else {
	  $sqlt = "SELECT DISTINCT categoria FROM qui_righe_rda WHERE stato_ordine = '4' AND negozio = '$shop' ORDER BY categoria ASC";
	}
  $risultt = mysql_query($sqlt) or die("Impossibile eseguire l'interrogazione9" . mysql_error());
  while ($rigat = mysql_fetch_array($risultt)) {
    if ($rigat[categoria] != "") {
