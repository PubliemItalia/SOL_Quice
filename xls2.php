...
$righe .= "</td>";
$righe .= "<td>";
$sumquery = "SELECT SUM(totale) as somma FROM qui_righe_rda WHERE id_rda = '$rigaaaa[id]'";
$resultb = mysql_query($sumquery);
list($somma) = mysql_fetch_array($resultb);
$totale_rda = $somma;
$righe .= numdecim($totale_rda);
$righe .= "</td>";
$righe .= "<td>";
...
$righe .= "</td>";
$righe .= "</tr>";
//inizia l'elenco delle righe
$Queryxx = "SELECT * FROM qui_righe_rda WHERE id_rda = '$rigaaaa[id]'";
$resultx = mysql_query($Queryxx);
while ($rowx = mysql_fetch_array($resultx)) {
$righe .= "<tr>";
$righe .= "<td>";
$righe .= $rowx[codice_art];
$righe .= "</td>";
$righe .= "<td>";
$righe .= str_replace("<br>"," ",tratta($rowx[descrizione]));
$righe .= "</td>";
$righe .= "<td>";
$righe .= intval($rowx[quant]);
$righe .= "</td>";
$righe .= "<td>";
$righe .= numdecim($rowx[totale]);
$righe .= "</td>";
$righe .= "<td>";
$righe .= $rowx[output_mode];
$righe .= "</td>";
$righe .= "<td>";
if ($rowx['data_chiusura'] > 0) {
$righe .= date("d/m/Y",$rowx['data_chiusura']);
}
$righe .= "</td>";
$righe .= "<td>";
$righe .= $rowx[pack_list];
$righe .= "</td>";
$righe .= "<td>";
$Queryyy = "SELECT * FROM qui_packing_list WHERE id = '$rowx[pack_list]'";
$resulty = mysql_query($Queryyy);
while ($rowy = mysql_fetch_array($resulty)) {
if ($rowy['data_spedizione'] > 0) {
$righe .= date("d/m/Y",$rowy['data_spedizione']);
}
}
$righe .= "</td>";
$righe .= "<td>";
switch ($rowx[stato_ordine]) {
	case "1":
	$righe .= "Inserita";
	break;
	case "2":
