...
$sqlq = "SELECT DISTINCT categoria1_it,categoria1_en FROM qui_prodotti_vivistore ORDER BY precedenza ASC";
$risultq = mysql_query($sqlq) or die("Impossibile eseguire l'interrogazione5" . mysql_error());
$num_categorie = mysql_num_rows($risultq);
$num_colonne = ceil(($num_categorie/$num_righeXcolonna));
while ($rigaq = mysql_fetch_array($risultq)) {
$add_categ = array_push($array_categorie_vivistore,$rigaq[$categoria1_lang]);
}
foreach ($array_categorie_vivistore as $sing_categ) {
  $contatore = $contatore +1;
  if ($contatore == 1) {
  //scrivo colonna della tabella contenitore
...
//$tabella_sottomenu_vivistore .= "<ul>";
  ...
  $tabella_sottomenu_vivistore .= "<a href=ricerca_prodotti.php?limit=".$limit."&page=".$page."&negozio=vivistore&categoria1=".$sing_categ.">".str_replace("_"," ",$sing_categ)."</a>";
...
<!--END SEARCH-->
	<div id="subMenu">
				<a href="lista_preferiti.php?a=1&Preferiti=1&negozio=Preferiti"><div class=subsub><?php echo $tasto_bookmark; ?></div></a>
				<a href=lista_rda.php?a=1&archive=1&lista=RdA&pers=1><div class=subsub><?php echo $tasto_archivio; ?></div></a>
				<a href=lista_rda.php?a=1&lista=RdA&pers=1><div class=subsub><?php echo $tasto_rda; ?></div></a>
				<?php
					if ($vis_magazzino == "1") {
        ...
        //echo "<a href=lista_rda.php?a=1&archive=1&lista=RdA><div class=subsub style=\"color:red;\">";
				...
				switch ($_SESSION[nome]) {
					case "Giorgio Gaiani":
					  echo "<a href=report_righe_mag.php><div class=subsub style=\"color:red;\">";
					break;
					case "AAA-MAGAZZINIERE":
					  echo "<a href=report_righe_mag_publiem.php><div class=subsub style=\"color:red;\">";
					break;
				}
