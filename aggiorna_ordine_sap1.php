...
foreach ($array_pl_ord as $pl_singolo) {
  ...
  $totale_fattura = $totale_fattura + $totale_pl;
	...
	$out_value .= "<a name=".$contatore.">";
	  ...
	  <div style="text-align: right; width: 130px; height: 20px; float: left; margin: 0px 10px;">Totale euro '.number_format($totale_fattura,2,",",".").'</div>';
	  $out_value .= '<div style="width: 500px; height: 20px; float: left; margin-top: 0px;">';
		$out_value .= '<div style="width:154px; height:22px; float:left;">
		<div style="margin:5px 5px 0px 0px; color: #dedede !important; font-weight: normal !important;">Ord. n. '.$n_ord.'</div>
		</div>';	
		$out_value .= '<div style="width:154px; height:22px; margin-left:10px; float:left;">
		<div style="margin:5px 5px 0px 0px; color: #dedede !important; font-weight: normal !important;">Fatt. n. '.$n_fatt.'</div>
		</div>';	
	  $out_value .= '</div>';
	  ...
	  $out_value .= "<div style=\"width: 50px; height: 20px; float: right; margin-right: 20px; cursor:pointer;\" onclick=\"PopupCenter('vista_fattura.php?mode=print&blocco_pl=".$blocco_pl."&unit=".$sing_unit."', 'myPop1',800,800);\">PDF</div>";
	  $out_value .= "</div>";
	$out_value .= "</a>";
