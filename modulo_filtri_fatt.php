...
$array_cat_flusso = array();
	$sqlt = "SELECT * FROM qui_buyer_negozi WHERE id_utente = '$_SESSION[id_utente]' ORDER BY preferenza ASC";
...
//echo "<br>";
//echo "blocco_negozi_buyer: ".$blocco_negozi_buyer."<br>";
//costruzione query
...
if (in_array($file_presente,$array_societa)) {
	  switch ($file_presente) {
		  ...
		  case "report_fatturazione.php":
			if ($clausole > 0) {
			  $tendSoc = "SELECT DISTINCT azienda_utente FROM qui_righe_rda WHERE dest_contab LIKE '%".$doc."%' AND ".$queryaccessoria;
			} else {
			  $tendSoc = "SELECT DISTINCT azienda_utente FROM qui_righe_rda WHERE dest_contab LIKE '%".$doc."%'";
			}
		  break;
		  case "ordini_sap.php":
...
if (in_array($file_presente,$array_categorie)) {
	switch ($file_presente) {
		...
		case "report_righe_nuovo.php":
			if ($shopDaModulo != "") {
			  $tendCATEG = "SELECT DISTINCT categoria FROM qui_righe_rda WHERE ".$queryaccessoria;
			} else {
			  if ($clausole > 0) {
				$tendCATEG = "SELECT DISTINCT categoria FROM qui_righe_rda WHERE ".$blocco_negozi_buyer." AND ".$queryaccessoria;
			  } else {
				$tendCATEG = "SELECT DISTINCT categoria FROM qui_righe_rda WHERE ".$blocco_negozi_buyer;
			  }
			}
		  $lungCATEG = strlen($tendCATEG);
		  $finaleCATEG = substr($tendCATEG,($lungCATEG-5),5);
